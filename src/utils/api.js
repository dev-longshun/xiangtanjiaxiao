 ...
 * 对应后端 AdminReviewController、AdminSchoolController
 */
export const adminAPI = {
  ...
  /**
   * 获取评价列表（管理员）
   * @param {string|null} status - 筛选状态：PENDING/APPROVED/REJECTED，null 表示全部
   * @returns {Promise<Result<Review[]>>}
   */
   ...
   * @param {boolean} approved - true=通过，false=驳回
   * @param {string} rejectReason - 驳回原因（驳回时必填）
   * @returns {Promise<Result<void>>}
   */
  reviewApproval(id, approved, rejectReason = "") {
    return request(`/api/admin/reviews/${id}/review`, {
      method: "POST",
  ...
  updateSchool(id, school) {
    return request(`/api/admin/schools/${id}`, {
      method: "PUT",
      body: JSON.stringify(school),
    });
  },
